...
Backburner.prototype = {
  ...
  setTimeout: function() {
    ...
    var i = searchTimer(executeAt, timers, 0, timers.length-1);

    timers.splice(i, 0, { executeAt: executeAt, fn: fn });
...
function executeTimers(self) {
  ...
  self.run(function() {
    i = searchTimer(now, timers, 0, timers.length-1);

    fns = timers.splice(0, i).map(function(t) { return t.fn; });
  ...
  if (timers.length) {
    updateLaterTimer(self, timers[0].executeAt, timers[0].executeAt - now);
  }
}
...
function searchTimer(time, timers, start, end) {
  if (start >= end) {
    if (typeof timers[start] !== 'undefined' && time > timers[start].executeAt) {
      return start+1;
    } else {
      return start;
    }
  ...
  if (time >= timers[middle].executeAt) {
    return searchTimer(time, timers, middle+1, end);
  } else {
    return searchTimer(time, timers, start, middle);
  }
}
